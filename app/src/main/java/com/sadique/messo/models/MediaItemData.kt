...
package com.sadique.messo.models
...
data class MediaItemData(
    ...
    val playbackRes: Int
        get() {
            return if (isPlaying) R.drawable.ic_pause
    ...
    companion object {
        ...
        fun timestampToMSS(context: Context, position: Long?): String {
            if (position == null || position < 0) return context.getString(R.string.duration_unknown)
            val totalSeconds = floor(position / 1E3).toInt()
            val minutes = totalSeconds / 60
            val remainingSeconds = totalSeconds - (minutes * 60)
            return context.getString(R.string.duration_format).format(minutes, remainingSeconds)
        }
