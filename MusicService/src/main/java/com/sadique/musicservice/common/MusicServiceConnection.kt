...
package com.sadique.musicservice.common
 ...
 *
 * Typically it's best to construct/inject dependencies either using DI or, as Messo does,
...
class MusicServiceConnection private constructor(context: Context) {
    ...
    private val _isConnected = MutableLiveData<Event<Resource<Boolean>>>()
        .apply { postValue(Event(Resource.loading(null))) }
    val isConnected: LiveData<Event<Resource<Boolean>>> = _isConnected

    private val _networkFailure = MutableLiveData<Event<Resource<Boolean>>>()
        .apply { postValue(Event(Resource.loading(null))) }
    val networkError: LiveData<Event<Resource<Boolean>>> = _networkFailure
    ...
    fun sendCommand(
        ...
        _mediaController.sendCommand(command, parameters, object : ResultReceiver(Handler()) {
    ...
    private inner class MediaBrowserConnectionCallback(private val context: Context) :
        ...
        override fun onConnected() {
            ...
            _shuffleMode.postValue(_mediaController.shuffleMode)
            _repeatMode.postValue(_mediaController.repeatMode)
        }
        ...
        override fun onConnectionSuspended() {
            _isConnected.postValue(Event(Resource.loading(null)))
        }
        ...
        override fun onConnectionFailed() {
            _isConnected.postValue(Event(Resource.error("", false)))
        }
    }
    ...
    private inner class MediaControllerCallback : MediaControllerCompat.Callback() {
        ...
        override fun onSessionEvent(event: String?, extras: Bundle?) {
            ...
            when (event) {
                NETWORK_FAILURE -> _networkFailure.postValue(Event(Resource.error(event, true)))
            }
        }
