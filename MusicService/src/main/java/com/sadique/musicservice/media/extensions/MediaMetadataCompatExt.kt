...
import com.sadique.musicservice.media.data.models.Song
import java.text.DateFormat
import com.google.android.exoplayer2.MediaItem as ExoplayerMediaItem
import com.google.android.exoplayer2.MediaMetadata as ExoplayerMediaMetadata
...
inline val MediaMetadataCompat.flag
    get() = this.getLong(METADATA_KEY_MESSO_FLAGS).toInt()
...
inline var MediaMetadataCompat.Builder.flag: Int
    ...
    get() = throw IllegalAccessException("Cannot get from MediaMetadataCompat.Builder")
    set(value) {
        putLong(METADATA_KEY_MESSO_FLAGS, value.toLong())
    }
 ...
 */
fun MediaMetadataCompat.Builder.from(song: Song, downloadStatus: Long): MediaMetadataCompat.Builder {
    ...
    trackCount = song.totalTrackCount
    ...
    displayIconUri = song.albumArtUri
 ...
 */
fun List<MediaMetadataCompat>.toConcatenatingMediaSource(
...
fun MediaMetadataCompat.toMediaQueueItem(): MediaQueueItem {
    val metadata = toCastMediaMetadata()
...
fun MediaMetadataCompat.toMediaItem(): ExoplayerMediaItem {
    val metadata = toExoplayerMediaMetadata()
    return ExoplayerMediaItem.Builder()
        .setMediaId(id!!)
...
private fun MediaMetadataCompat.toExoplayerMediaMetadata(): ExoplayerMediaMetadata {
    val releaseDate = date?.let { DateFormat.getInstance().parse(it) }
    return ExoplayerMediaMetadata.Builder()
        .setTitle(title)
        .setArtist(artist)
        .setAlbumTitle(album)
        .setDisplayTitle(title)
        .setArtworkUri(albumArtUri)
        .setArtworkData(albumArt.toByteArray(), ExoplayerMediaMetadata.PICTURE_TYPE_FILE_ICON)
        .setAlbumArtist(albumArtist)
        .setComposer(composer)
        .setMediaUri(mediaUri)
        .setTrackNumber(trackCount.toInt())
        .setDiscNumber(discNumber.toInt())
        .setReleaseDay(releaseDate?.day)
        .setReleaseMonth(releaseDate?.month)
        .setReleaseYear(releaseDate?.year)
        .build()
}
 ...
 */
const val METADATA_KEY_MESSO_FLAGS = "com.sadique.messo.media.METADATA_KEY_MESSO_FLAGS"
